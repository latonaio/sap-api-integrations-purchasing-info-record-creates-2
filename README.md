...
sap-api-integrations-purchasing-info-record-creates は、外部システム(特にエッジコンピューティング環境)をSAPと統合することを目的に、SAP API で購買情報レコードデータを登録するマイクロサービスです。  
...
以下の sample.json の例は、SAP 購買情報レコード の 一般データ が登録された結果の JSON の例です。  
